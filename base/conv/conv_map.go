...
import (
	"github.com/wuqishan/go_kit/base/empty"
	"github.com/wuqishan/go_kit/base/jsonx"
	"github.com/wuqishan/go_kit/base/utils"
	"reflect"
	"strings"
)
...
func doMapConvert(value interface{}, recursive recursiveType, mustMapReturn bool, option ...MapOption) map[string]interface{} {
	...
	case string:
		...
		if len(r) > 0 && r[0] == '{' && r[len(r)-1] == '}' {
			if err := jsonx.UnmarshalUseNumber([]byte(r), &dataMap); err != nil {
				return nil
			}
		} else {
			return nil
	...
	case []byte:
		// If it is a JSON string, automatically unmarshal it!
		if len(r) > 0 && r[0] == '{' && r[len(r)-1] == '}' {
			if err := jsonx.UnmarshalUseNumber(r, &dataMap); err != nil {
				return nil
			}
		} else {
			return nil
